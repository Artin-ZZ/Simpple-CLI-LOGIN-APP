...
# Ticket Id Unique Generator
def random_tkid(chars: str = string.ascii_uppercase + string.digits):
    result  = "TKA"
    result  += f'{round(time.time() * 1000)}'
    return result

# Car Track Id Unique GENERATOR
def randomTrackId(chars: str = string.ascii_uppercase + string.digits):
    result = "TRKD"
    result += f"{round(time.time() * 1000)}"
    return result